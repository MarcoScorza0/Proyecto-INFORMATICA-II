...
void init_mcu(void){
   ...
   //salidas_leds.modo=avr_GPIO_mode_Output;
   //salidas_leds.port =  SALIDAS_LEDS_PORT;
   //salidas_leds.pines = LED_1_PIN | LED_2_PIN;
   //nit_gpio(salidas_leds);
   
   GpioInitStructure_AVR entradas_adc;
entradas_adc.modo = avr_GPIO_mode_Input;
entradas_adc.port = avr_GPIO_C;       // Puerto C
entradas_adc.pines = avr_GPIO_PIN_0;  // Pin C0 (ADC0)
